...
import torch
import numpy as np
from inner_product.simulator_connect import InnerProductSimulator
import math
import csv
...
def test_calc():
    ...
    for i, test_case in enumerate(test_cases):
        ...
        # 测试两种分组模式
        for group_mode in ['diagonal', 'row']:
            print(f"\n--- 测试 {group_mode} 模式 ---")
            
            # 使用模拟器完整计算
            simulator = InnerProductSimulator(group_mode=group_mode)
            padding = 1
            stride = 1
            ...
            # 计算加速比
            speedup = latency_estimation_nvdla / simulator.compute_cycles
            
            # 打印结果
            print(f"模拟器输入形状: {input_tensor.shape}")
            print(f"模拟器输出形状: {output_tensor.shape}")
            print(f"模拟器总周期: {simulator.total_cycles}")
            print(f"模拟器计算周期: {simulator.compute_cycles}")
            print(f"理论计算周期: {theorical_cycles}")
            ...
            results.append({
                ...
                'dense': dense,
                'simulator_cycles': simulator.total_cycles,
                'compute_cycles': simulator.compute_cycles,
    ...
    # 将结果保存为CSV文件
    with open(f'../output/simulator_connect_test_results.csv', 'w', newline='') as csvfile:
        fieldnames = ['test_case', 'file', 'Cout', 'group_mode', 'input_shape', 'dense', 'simulator_cycles', 'compute_cycles', 'theorical_cycles', 'dla_cycles', 'speedup', 'total_data_bits', 'total_data_kb']
    ...
    print(f"\n测试完成，结果已保存到 output/simulator_connect_test_results.csv 文件中")
