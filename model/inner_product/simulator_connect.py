...
class PE:
    ...
    def process(self, act):
        ...
        self.stats.compute_cycles += cycle_num  
        
        return psum, cycle_num 
...
class Core:
    ...
    """
    def __init__(self, name, cout_idx, num_pes=64, adder_num=2, group_mode='diagonal'):
        self.name = name
        ...
        # 初始化PE列表
        self.pes = [PE(torch.zeros(9), adder_num, f"pe{i}", i, group_mode) for i in range(num_pes)]
...
class InnerProductSimulator:
    ...
    """
    def __init__(self, group_mode='diagonal'):
        ...
        self.total_cycles = 0
        self.lif_num     = 1
        self.group_mode = group_mode  # 分组模式：'diagonal' 或 'row'

        self.processed_cin = self.num_pes  #每个pe对应一个cin
        self.cores = [Core(f"core{i}", i, self.num_pes, self.adder_num, self.group_mode) for i in range(self.num_cores)]
    ...
    def set_group_mode(self, group_mode):
        ...
        Args:
            group_mode: 分组模式，'diagonal' 或 'row'
        """
        self.group_mode = group_mode
        # 更新所有core和PE的分组模式
        for core in self.cores:
            core.group_mode = group_mode
            for pe in core.pes:
                pe.group_mode = group_mode
...
# 测试5D输入和4D权重 - diagonal模式
    print("\n=== 5D输入和4D权重测试 - diagonal模式 ===")
    ...
    simulator = InnerProductSimulator(group_mode='diagonal')
    simulator.num_pes = 8
...
# 测试5D输入和4D权重 - row模式
    print("\n=== 5D输入和4D权重测试 - row模式 ===")
    simulator.set_group_mode('row')
    simulator.print_core_info = True  # 重新启用打印
    output_tensor, stats = simulator.run_convolution(input_tensor, kernel_tensor, padding=padding, stride=stride)
