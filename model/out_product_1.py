...
import time
import math
from out_product import convolution_out_product, convolution_with_padding_out_product


def main():
    start_time = time.time()
    #读取npy文件,lif的输出，实际上是conv1的输入
    input_tensor = np.load('../conv/B=128/patch_embed_proj_conv1_input.npy')
    ...
    output_tensor = np.load('../conv/B=128/patch_embed_proj_conv1_output.npy')
    output_tensor = torch.from_numpy(output_tensor)
    TB,Cout,oh,ow = output_tensor.shape  #4,128,96,32,32
