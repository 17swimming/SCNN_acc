...
#使用真实数据测试
    ...
    input_tensor = np.load('../conv/patch_embed_proj_lif_output.npy')
    input_tensor = torch.from_numpy(input_tensor)
    T,B,Cin,h,w = input_tensor.shape
    print("输入tenor的稀疏度：", (input_tensor == 0).sum() / input_tensor.numel())

    output_tensor = np.load('../conv/patch_embed_proj_lif1_output.npy')
    output_tensor = torch.from_numpy(output_tensor)
    T,B,Cout,oh,ow = output_tensor.shape  #4,128,96,32,32
