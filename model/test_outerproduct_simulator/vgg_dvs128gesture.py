...
def test_calc():
    ...
    test_cases = [
        {'file': os.path.join(current_dir, '../../conv/VGG_DVSGesTure/features.0.fwd.module.0_input.npy'), 'Cout': 64},  
        {'file': os.path.join(current_dir, '../../conv/VGG_DVSGesTure/features.1.fwd.module.0_input.npy'), 'Cout': 128},
        {'file': os.path.join(current_dir, '../../conv/VGG_DVSGesTure/features.3.fwd.module.0_input.npy'), 'Cout': 256},
        {'file': os.path.join(current_dir, '../../conv/VGG_DVSGesTure/features.4.fwd.module.0_input.npy'), 'Cout': 256},
        {'file': os.path.join(current_dir, '../../conv/VGG_DVSGesTure/features.6.fwd.module.0_input.npy'), 'Cout': 512},  
        {'file': os.path.join(current_dir, '../../conv/VGG_DVSGesTure/features.7.fwd.module.0_input.npy'), 'Cout': 512},
        {'file': os.path.join(current_dir, '../../conv/VGG_DVSGesTure/features.9.fwd.module.0_input.npy'), 'Cout': 512},
        {'file': os.path.join(current_dir, '../../conv/VGG_DVSGesTure/features.10.fwd.module.0_input.npy'), 'Cout': 512},
    ]
    ...
    output_folder = '../../output/simulator_outproduct/vgg_dvs128gesture'
    ...
    for i, test_case in enumerate(test_cases):
        ...
        # 使用模拟器完整计算,input_tensor.shape=[T,B,C,H,W]
        padding = 1
        stride = 1
        num_cores = 8
        num_pus = 16
        ...
        generate_pe_utilization_heatmap(output_folder,pe_utilization,filename=f'{test_case["file"].split("/")[-1].split(".")[0]}_{i}_pe_utilization.png')
    ...
    if all_pe_cycles:
        ...
        print(f"overall_avg_pe_utilization的shape: {overall_avg_pe_utilization.shape}")
        # 计算各维度平均值
        avg_by_pe = torch.mean(overall_avg_pe_utilization, dim=(0))  # 对测试用例取平均
        print(f"\n=== 总体平均PE利用率 ===")
    ...
    filename = f'{output_folder}/core{num_cores}_pu{num_pus}_dla{avg_speedup:.2f}_sato{avg_sato_speedup:.2f}_simulator_mapping_test_results.csv'
    ...
    plot_cycle_ratio(output_folder, results, num_cores, num_pus)
    # 绘制速度对比图（以DLA为基准）
    plot_speedup_comparison(output_folder, results, num_cores, num_pus)


def plot_cycle_ratio(output_folder, results, num_cores, num_pus):
    """
    ...
    filename = f'{output_folder}/cycle_comparison_cores{num_cores}_pu{num_pus}.png'
    plt.tight_layout()
...
def plot_speedup_comparison(output_folder, results, num_cores, num_pus):
    """
    ...
    os.makedirs(output_folder, exist_ok=True)
    filename = f'{output_folder}/speedup_comparison_cores{num_cores}_pu{num_pus}.png'
    plt.tight_layout()
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"\n已保存速度对比图到: {filename}")
...
def generate_pe_utilization_heatmap(output_folder, pe_utilization,filename='pe_utilization.png'):
        """
        生成PE利用率热图
